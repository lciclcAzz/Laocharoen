...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="idx-content-layout">';
        $result .= '<div class="idx-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="idx-layout-cell' . ('idx-block' == $style ? ' idx-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="idx-post">' string in it.
     * Then it wraps the content of the buffer with the idx-post.
     */
    public function componentWrapper()
    {
        ...
        if (false === strpos($content, '<div class="idx-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'idx-main').addClass('idx-postcontent');
            }
        });
    }
});
