...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the idx-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"idx-post" . $classes . "\">");
            artxFragmentBegin("<h2 class=\"idx-postheader\">");
            ...
            artxFragmentEnd("</h2>");
            artxFragmentBegin("<div class=\"idx-postheadericons idx-metadata-icons\">");
            if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
                foreach ($data['metadata-header-icons'] as $icon)
                    artxFragment('', $icon, '', ' | ');
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"idx-postcontent idx-postcontent-0 clearfix\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"idx-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"idx-blockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"idx-vmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"idx-vmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"idx-vmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'idx-button'
     *   artxFindTags($html, array('input' => array('class' => '!idx-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'idx-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!idx-button', '!'))))
     *  Select inputs with class = button but != 'idx-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!idx-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'idx-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'idx-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    function artxPostprocessContent($content)
    {
        ...
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!idx-button')),
            'button' => array('class' => array('button', '!idx-button')), 'button' => array('class' => array('btn', '!idx-button'))));
        return $content;
    }
