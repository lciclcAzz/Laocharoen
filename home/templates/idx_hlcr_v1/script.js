...
jQuery(function ($) {
    ...
    $('.idx-slider .idx-slide-item').each(function () {
...
jQuery(function ($) {
    ...
    // ie8
    if (!$.browser.msie || $.browser.version > 8) return;
    $('.idx-shapes').css('z-index', 1);
    
    // ie7
    if (!$.browser.msie || $.browser.version > 7) return;
    var textblockTexts = $('.idx-textblock > div');
...
var navigatorResizeHandler = (function ($) {
    ...
    return function (responsiveDesign) {
        ...
        $(".idx-slider").each(function () {
            var slider = $(this);
            var sliderWidth = slider.width();
            var nav = slider.siblings(".idx-slidenavigator");
            if (nav.length) {
                // left offset
...
jQuery(window).bind("resize", (function ($) {
    ...
    return function () {
        if (typeof responsiveDesign !== "undefined" && responsiveDesign.isResponsive)
            return;
        var sheetLeft = $(".idx-sheet").offset().left;
        $("header.idx-header #idx-flash-area").each(function () {
            var object = $(this);
...
jQuery(function($) {
    "use strict";
    $('nav.idx-nav').addClass("desktop-nav");
});
...
jQuery(function ($) {
    ...
    $('ul.idx-hmenu>li:not(:first-child)').each(function () { $(this).prepend('<span class="idx-hmenu-separator"> </span>'); });
});

jQuery(function ($) {
    "use strict";
    $("ul.idx-hmenu a:not([href])").attr('href', '#').click(function (e) { e.preventDefault(); });
});
...
jQuery(function () {
    ...
    setHMenuOpenDirection({
        container: "div.idx-sheet",
        defaultContainer: "#idx-main",
        menuClass: "idx-hmenu",
        leftToRightClass: "idx-hmenu-left-to-right",
        rightToLeftClass: "idx-hmenu-right-to-left"
    });
});
...
var megaMenuCreate = (function ($) {
    ...
    return function () {
        ...
        sheetWidth -= (parseInt($(".idx-hmenu").parents("nav").css("padding-left").replace("px", ""), 10) || 0) * 2;

        // reset
        $(".idx-hmenu .idx-hmenu-mega-menu").removeClass("idx-hmenu-mega-menu").css("width", "").css("left", "").css("right", "")
        ...
        $(".idx-hmenu>li>ul").each(function() {
            var ul = $(this);
            ul.addClass("idx-hmenu-mega-menu");

            var ltr = !ul.hasClass("idx-hmenu-right-to-left");
            var widths = [];
...
jQuery(function ($) {
    ...
    $(window).bind('resize', function () {
        var bh = $('body').height();
        var mh = 0;
        var c = $('div.idx-content');
        c.removeAttr('style');

        $('#idx-main').children().each(function() {
    ...
    if ($.browser.msie && parseInt($.browser.version, 10) < 8) {
        $(window).bind('resize', function() {
            var c = $('div.idx-content');
            var s = c.parent().children('.idx-layout-cell:not(.idx-content)');
            var w = 0;
            c.hide();
...
jQuery(function($) {
    ...
    $('ul.idx-vmenu li:not(:first-child),ul.idx-vmenu li li li:first-child,ul.idx-vmenu>li>ul').each(function () { $(this).append('<div class="idx-vmenu-separator"> </div><div class="idx-vmenu-separator-bg"> </div>'); });
});
jQuery(function() {
    "use strict";
    setPopupVMenuOpenDirection({container: "div.idx-sheet", defaultContainer: "#idx-main", vMenuClass: "idx-vmenu", leftToRightClass: "idx-vmenu-left-to-right", rightToLeftClass: "idx-vmenu-right-to-left"});
    fixPopupVMenu({vMenuClass: "idx-vmenu", vMenuLayoutCellClass: "idx-layout-cell"});
});
...
var artButtonSetup = (function ($) {
    ...
    return (function (className) {
        $.each($("a." + className + ", button." + className + ", input." + className), function (i, val) {
            var b = $(val);
            if (!b.hasClass('idx-button')) {
                b.addClass('idx-button');
            }
            if (b.is('input')) {
...
jQuery(function ($) {
    "use strict";
    var rssIcons = $(".idx-rss-tag-icon");
    if (rssIcons.length){
        fixRssIconLineHeight("idx-rss-tag-icon");
        if ($.browser.msie && parseInt($.browser.version, 10) < 9) {
...
var ThemeLightbox = (function ($) {
    ...
    return (function () {
        var current;
        var images = $(".idx-lightbox");

        this.init = function (ctrl) {
            $(".idx-lightbox").live("click", { _ctrl: ctrl }, function (e) {
            ...
            $(".idx-lightbox-wrapper .arrow.left:not(.disabled)").live("click", function () {
                move(current - 1);
            });

            $(".idx-lightbox-wrapper .arrow.right:not(.disabled)").live("click", function () {
                move(current + 1);
            });

            $(".idx-lightbox-wrapper .active").live("click", function () {
                move(current + 1);
            });

            $(".idx-lightbox-wrapper .close").live("click", function () {
                close();
            });
        };
        ...
        function show(src) {
            ...
            var imgContainer = $('.idx-lightbox-wrapper');
            if (imgContainer.length === 0) {
                imgContainer = $('<div class="idx-lightbox-wrapper">').css('line-height', $(window).height() + "px")
                    .appendTo($("body"));
            }
            
            var img = $('<img class="idx-lightbox-image active" src="' + getFullImgSrc($(src).attr("src")) + '">');
            ...
            bindMouse($(".idx-lightbox-wrapper .arrow").add(img).add(imgContainer));
        }
        ...
        function move(index) {
            ...
            $(".idx-lightbox-wrapper .idx-lightbox-image:not(.active)").remove();

            var active = $(".idx-lightbox-wrapper .active");
            var target = $('<img class="idx-lightbox-image" alt="" src="' + getFullImgSrc($(images[current]).attr("src")) + '" />');
            ...
            bindMouse($(".idx-lightbox-wrapper").add(target));
        ...
        function showArrows() {
            if ($(".idx-lightbox-wrapper .arrow").length === 0) {
                $(".idx-lightbox-wrapper").append($('<div class="arrow left"><div class="arrow-t ccw"> </div><div class="arrow-b cw"> </div><div class="arrow-left-alt">&#8592;</div></div>').css("top", $(window).height() / 2 - 40));
                $(".idx-lightbox-wrapper").append($('<div class="arrow right"><div class="arrow-t cw"> </div><div class="arrow-b ccw"> </div><div class="arrow-right-alt">&#8594;</div></div>').css("top", $(window).height() / 2 - 40));
            }

            if (current === 0) {
                $(".idx-lightbox-wrapper .arrow.left").addClass("disabled");
            } else {
                $(".idx-lightbox-wrapper .arrow.left").removeClass("disabled");
            }

            if (current === images.length - 1) {
                $(".idx-lightbox-wrapper .arrow.right").addClass("disabled");
            } else {
                $(".idx-lightbox-wrapper .arrow.right").removeClass("disabled");
            }
        }

        function showError(enable) {
            if (enable) {
                $(".idx-lightbox-wrapper").append($('<div class="lightbox-error">The requested content cannot be loaded.<br/>Please try again later.</div>')
                        .css({ "top": $(window).height() / 2 - 60, "left": $(window).width() / 2 - 170 }));
            } else {
                $(".idx-lightbox-wrapper .lightbox-error").remove();
            }
        }

        function showLoader(enable) {
            if (!enable) {
                $(".idx-lightbox-wrapper .loading").remove();
            }
            else {
                $('<div class="loading"> </div>').css({ "top": $(window).height() / 2 - 16, "left": $(window).width() / 2 - 16 }).appendTo($(".idx-lightbox-wrapper"));
            }
        }

        var close = function () {
            $(".idx-lightbox-wrapper").remove();
        };
...
(function($) {
    ...
    var Slider = function (element, settings) {
        ...
        this.move = function (direction, next) {
            ...
            if (!nextItem.length) {
                nextItem = element.find(".idx-slide-item")[reset]();
        ...
        if (this.settings.hover) {
            ...
            element.add(this.settings.navigator)
                   .add(element.siblings(".idx-shapes")).hover(function () {
...
(function ($) {
    ...
    $(function ($) {
        var links = $('a.idx-rss-tag-icon[href="#"], a.idx-rss-tag-icon[href=""]'),
    ...
    $(function ($) {
        $('.idx-messages').each(function () {
            $(this).css('display', $(this).find('dl').length ? '' : 'none');
        });
    });

    $(function ($) {
        $('.idx-search-button').each(function (i, val) {
            var element = $(val),
            ...
            if (element.hasClass('idx-button')) {
                element.removeClass('idx-button');
            }
        });
    });
